        ...
        rows.append(
            {
                ...
                "protocol": "TCP",
                "packet_count": int(rng.integers(1, 4)),
        ...
        rows.append(
            {
                ...
                "failed_connections": 0,
                "connection_rate": float(rng.uniform(0.1, 0.3)),
                "authentication_failures": 0,
                "service": "https",
                "status": "SUCCESS",
                "label": "anomaly",
...
def generate_dataset(
    n_normal: int = 4000, anomaly_fraction: float = 0.05, seed: int = RNG_SEED
) -> pd.DataFrame:
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=str,
        default=str(Path(__file__).resolve().parents[1] / "data" / "sample" / "network_events.csv"),
    )
